...
static void ApplyWordsSettings(const ConfigSnapshot* snapshot) {
    WORD_DISPLAY_ENABLED = snapshot->wordsEnabled;
    WORD_SWITCH_INTERVAL_SEC = snapshot->wordsSwitchIntervalSec;
    if (WORD_SWITCH_INTERVAL_SEC < 0) WORD_SWITCH_INTERVAL_SEC = 0;

    WORD_SHOW_PHONETIC = snapshot->wordsShowPhonetic;
    WORD_PHONETIC_MODE = snapshot->wordsPhoneticMode;
    if (WORD_PHONETIC_MODE < 0) WORD_PHONETIC_MODE = 0;
    if (WORD_PHONETIC_MODE > 2) WORD_PHONETIC_MODE = 2;

    WORD_SHOW_CHINESE = snapshot->wordsShowChinese;
    WORD_CHINESE_MAX_LEN = snapshot->wordsChineseMaxLen;
    if (WORD_CHINESE_MAX_LEN < 0) WORD_CHINESE_MAX_LEN = 0;

    /* Ensure subsystem is initialized if enabled */
