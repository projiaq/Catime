...
BOOL CollectCurrentConfig(ConfigWriteItem* items, int* count) {
    ...
    /* Words section */
    safe_strncpy(items[idx].section, INI_SECTION_WORDS, sizeof(items[idx].section));
    safe_strncpy(items[idx].key, "ENABLED", sizeof(items[idx].key));
    safe_strncpy(items[idx].value, WORD_DISPLAY_ENABLED ? "TRUE" : "FALSE", sizeof(items[idx].value));
    idx++;

    safe_strncpy(items[idx].section, INI_SECTION_WORDS, sizeof(items[idx].section));
    safe_strncpy(items[idx].key, "SWITCH_INTERVAL_SEC", sizeof(items[idx].key));
    snprintf(items[idx].value, sizeof(items[idx].value), "%d", WORD_SWITCH_INTERVAL_SEC);
    idx++;

    safe_strncpy(items[idx].section, INI_SECTION_WORDS, sizeof(items[idx].section));
    safe_strncpy(items[idx].key, "SHOW_PHONETIC", sizeof(items[idx].key));
    safe_strncpy(items[idx].value, WORD_SHOW_PHONETIC ? "TRUE" : "FALSE", sizeof(items[idx].value));
    idx++;

    safe_strncpy(items[idx].section, INI_SECTION_WORDS, sizeof(items[idx].section));
    safe_strncpy(items[idx].key, "PHONETIC_MODE", sizeof(items[idx].key));
    snprintf(items[idx].value, sizeof(items[idx].value), "%d", WORD_PHONETIC_MODE);
    idx++;

    safe_strncpy(items[idx].section, INI_SECTION_WORDS, sizeof(items[idx].section));
    safe_strncpy(items[idx].key, "SHOW_CHINESE", sizeof(items[idx].key));
    safe_strncpy(items[idx].value, WORD_SHOW_CHINESE ? "TRUE" : "FALSE", sizeof(items[idx].value));
    idx++;

    safe_strncpy(items[idx].section, INI_SECTION_WORDS, sizeof(items[idx].section));
    safe_strncpy(items[idx].key, "CHINESE_MAX_LEN", sizeof(items[idx].key));
    snprintf(items[idx].value, sizeof(items[idx].value), "%d", WORD_CHINESE_MAX_LEN);
    idx++;
