...
void GetTimeText(wchar_t* buffer, size_t bufferSize) {
    ...
    /* Append word suffix (built-in) */
    if (WORD_DISPLAY_ENABLED) {
        wchar_t suffix[512] = {0};
        WordsDisplay_FormatSuffix(suffix, 512);
        if (suffix[0]) {
            size_t curLen = wcslen(buffer);
            if (curLen < bufferSize - 1) {
                wcsncat(buffer, suffix, bufferSize - curLen - 1);
            }
        }
    }
